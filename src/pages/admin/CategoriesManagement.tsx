...
const CategoriesManagement = () => {
  ...
  const loadCategories = async () => {
    try {
      const { data, error } = await supabase.from('donation_categories').select('*').order('display_order', { ascending: true });
  ...
  const loadQuranicVerse = async () => {
    try {
      const { data, error } = await supabase.from('kiosk_settings').select('quranic_verse, quranic_verse_surah').limit(1).maybeSingle();
      if (error) throw error;
      setQuranicVerse(data?.quranic_verse || 'وَمَا تُنفِقُوا مِنْ خَيْرٍ فَإِنَّ اللَّهَ بِهِ عَلِيمٌ');
      setQuranicVerseSurah((data as any)?.quranic_verse_surah || 'البقرة');
    } catch (error: any) {
      toast({ title: "Error loading Quranic verse", description: error.message, variant: "destructive" });
    }
  };
  ...
  const saveQuranicVerse = async () => {
    setSavingVerse(true);
    try {
      const { data: settingsData } = await supabase.from('kiosk_settings').select('id').limit(1).single();
      const { error } = await supabase
        .from('kiosk_settings')
        .update({ quranic_verse: quranicVerse, quranic_verse_surah: quranicVerseSurah } as any)
        .eq('id', settingsData?.id);
      if (error) throw error;
      toast({ title: "Quranic verse updated successfully" });
    } catch (error: any) {
      toast({ title: "Error saving Quranic verse", description: error.message, variant: "destructive" });
  ...
  const handleIconChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (file.size > 100 * 1024) { toast({ title: "File too large", description: "Icon must be less than 100KB", variant: "destructive" }); return; }
    if (!['image/jpeg', 'image/jpg', 'image/png'].includes(file.type)) { toast({ title: "Invalid file type", description: "Only JPG, JPEG, and PNG files are allowed", variant: "destructive" }); return; }
    setIconFile(file);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      loadCategories();
    } catch (error: any) { toast({ title: "Error deleting category", description: error.message, variant: "destructive" }); }
  };

  const toggleVisibility = async (id: string, currentVisibility: boolean) => {
    try {
      const { error } = await supabase.from('donation_categories').update({ is_visible: !currentVisibility }).eq('id', id);
      if (error) throw error;
      toast({ title: "Visibility updated" });
      loadCategories();
    } catch (error: any) { toast({ title: "Error updating visibility", description: error.message, variant: "destructive" }); }
  };
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      const updates = updatedCategories.map((cat, index) => ({ id: cat.id, display_order: index + 1 }));
      for (const update of updates) {
        const { error } = await supabase.from('donation_categories').update({ display_order: update.display_order }).eq('id', update.id);
        if (error) throw error;
      }
      toast({ title: "Category order updated successfully" });
    } catch (error: any) { toast({ title: "Error updating category order", description: error.message, variant: "destructive" }); loadCategories(); }
  };
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <Button variant="outline" size="icon" onClick={() => navigate('/admin')}><ArrowLeft className="w-4 h-4" /></Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <Card className="p-6 md:col-span-2">
            ...
            <p className="text-sm text-muted-foreground mb-4">This verse appears at the top of the kiosk homepage</p>
            <div className="space-y-4">
              <div>
                <Label htmlFor="quranic_verse">Quranic Verse (Arabic)</Label>
                <Textarea id="quranic_verse" value={quranicVerse} onChange={(e) => setQuranicVerse(e.target.value)} placeholder="الآية القرآنية التي تظهر في الصفحة الرئيسية" rows={3} className="text-right" />
              </div>
              <div>
                <Label htmlFor="quranic_surah">Surah Name / اسم السورة</Label>
                <Select value={quranicVerseSurah} onValueChange={setQuranicVerseSurah}>
                  <SelectTrigger id="quranic_surah">
              ...
              <Button onClick={saveQuranicVerse} disabled={savingVerse}>
          ...
          <Card className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">Enter a unique reference code (appears in reports and CSV exports)</p>
              </div>
              <div>
                <Label htmlFor="title">Title (Arabic)</Label>
                <Input id="title" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} required placeholder="عنوان الفئة" />
              </div>
              <div>
                <Label htmlFor="title_en">Title (English)</Label>
                <Input id="title_en" value={formData.title_en} onChange={(e) => setFormData({ ...formData, title_en: e.target.value })} placeholder="Category title in English" />
              </div>
              <div>
              ...
              <div>
                ...
                <div className="mt-2">
                  ...
                  <input ref={iconInputRef} type="file" accept="image/jpeg,image/jpg,image/png" onChange={handleIconChange} className="hidden" />
                  <p className="text-sm text-muted-foreground">Only JPG, JPEG, and PNG files. Maximum size: 100 KB</p>
                </div>
                {iconPreview && (
                  <div className="mt-2 relative w-24 h-24">
                    <img src={iconPreview} alt="Icon preview" className="w-full h-full object-cover rounded-lg border-2 border-border" />
          ...
          {/* Categories List */}
          <div className="space-y-4">
            <h2 className="text-xl font-bold">Existing Categories ({categories.length}/8)</h2>
            {loading ? (<p>Loading...</p>) : (
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                <SortableContext items={categories.map(c => c.id)} strategy={verticalListSortingStrategy}>
                  {categories.map((category) => (
                    <SortableCategory key={category.id} category={category} onToggleVisibility={toggleVisibility} onEdit={handleEdit} onDelete={handleDelete} />
                  ))}
                </SortableContext>
              </DndContext>
            )}
