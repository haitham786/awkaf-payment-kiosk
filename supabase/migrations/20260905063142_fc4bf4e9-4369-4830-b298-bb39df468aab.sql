...
DROP POLICY IF EXISTS "Admins can view targets" ON public.targets;
CREATE POLICY "Admins can view targets" ON public.targets
...
BEGIN
  ...
  cause_cur AS (
    SELECT COALESCE(dc.title, n.category_reference, n.category::text) AS name,
